...
func (s FDBStatus) ExportProcesses() {
	// We .Reset() all the metrics here in order that we clear out any no-longer running processes.
	// Without this, we'll continue to build up metrics over time for processes that no longer exist
	// and the only way to clear those out otherwise is to restart.
	processCPUInfo.Reset()
	processDiskInfoBusy.Reset()
	processDiskInfoFreeBytes.Reset()
	processDiskInfoReadHZ.Reset()
	processDiskInfoReadTotal.Reset()
	processDiskInfoTotalBytes.Reset()
	processDiskInfoWriteHZ.Reset()
	processDiskInfoWritesTotal.Reset()
	processMemoryInfoAvailableBytes.Reset()
	processMemoryInfoLimitBytes.Reset()
	processMemoryInfoUnusedMemory.Reset()
	processMemoryInfoUsedBytes.Reset()
	processNetworkConnectionError.Reset()
	processNetworkConnectionsClosed.Reset()
	processNetworkConnectionsEstablished.Reset()
	processNetworkCurrentConnection.Reset()
	processNetworkReceived.Reset()
	processNetworkSent.Reset()
	logRoleDataVersion.Reset()
	logRoleDurableBytesHZ.Reset()
	logRoleDurableBytesCounter.Reset()
	logRoleAvailableBytes.Reset()
	logRoleFreeBytes.Reset()
	logRoleTotalBytes.Reset()
	logRoleUsedBytes.Reset()
	logRoleQueueAvailableBytes.Reset()
	logRoleQueueFreeBytes.Reset()
	logRoleQueueTotalBytes.Reset()
	logRoleQueueUsedbytes.Reset()
	storageRoleBytesQueriedHz.Reset()
	storageRoleBytesQueriedCounter.Reset()
	storageRoleDataLagSeconds.Reset()
	storageRoleDataLagVersions.Reset()
	storageRoleDurabilityLagSeconds.Reset()
	storageRoleDurabilityLagVersions.Reset()
	storageRoleDurableVersions.Reset()
	storageRoleFinishedQueriesCounter.Reset()
	storageRoleFinishedQueriesHz.Reset()
	storageRoleInputBytesCounter.Reset()
	storageRoleInputBytesHz.Reset()
	storageRoleKeysQueriedCounter.Reset()
	storageRoleKeysQueriedHz.Reset()
	storageRoleMutationsCounter.Reset()
	storageRoleMutationHz.Reset()
	storageRoleMutationsBytesCounter.Reset()
	storageRoleMutationBytesHz.Reset()
	storageRoleTotalQueriesCounter.Reset()
	storageRoleTotalQueriesHz.Reset()
	storageRoleQueryMaxQueue.Reset()
	storageRoleStoredBytes.Reset()
